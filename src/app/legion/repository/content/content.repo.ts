...
export abstract class ContentRepository {
    ...
    this.getCurrentPageObserver().subscribe({
      next: (youtubeVideoPage) => {
        if (youtubeVideoPage.id !== undefined && youtubeVideoPage.id !== '') {
          console.log(
            '🚀 ~ extract.content.repo fetched object successfully',
            youtubeVideoPage
          );
          this.currentPage = youtubeVideoPage;
        } else {
          console.log(
            '🚀 ~ extract.content.repo fetched object created unsuccessfully ID MISSING',
            youtubeVideoPage
          );
        }
      },
  ...
  getDefaultVideoNichesObserver(): Observable<VideoNiche[]> {
    this.translate.onLangChange
      .pipe(
        concatMap((event: LangChangeEvent) =>
          this.translate.getTranslation(event.lang)
        )
      )
      .subscribe({
        next: (res) => {
  ...
  getCurrentPage(id: string): Observable<YoutubeVideoPage> {
    ...
    if (id === '' && this.currentPage !== undefined) {
      ...
      return this.firestoreRepository
        .getUsersDocument<YoutubeVideoPage>(this.collectionPath, id)
        .pipe(
  ...
  getGeneratedAudioUrl(parentVideoId: string) {
    this.firestoreRepository
      .getUsersDocument<YoutubeVideoPage>(this.collectionPath, parentVideoId)
      .pipe(
        takeUntil(this.currentPageSubject),
        //filiering for undefined
        filter((data) => !!data),
  ...
  updateTitle(newTitle: string) {
    this.firestoreRepository
      ...
      .catch((err) =>
        console.log(
          '❤️‍🔥 ~ file: extractcontent.repo.ts ~ line 48 ~ ExtractContentRepository ~ err',
          err
        )
      );
  }
  ...
  updateDescription(newDescription: string) {
    this.firestoreRepository
      ...
      .catch((err) =>
        console.log(
          '❤️‍🔥 ~ file: extractcontent.repo.ts ~ line 56 ~ ExtractContentRepository ~ err',
          err
        )
      );
  }
  ...
  updateTags(newTags: string[]) {
    this.firestoreRepository
      ...
      .catch((err) =>
        console.log(
          '❤️‍🔥 ~ file: extractcontent.repo.ts ~ line 64 ~ ExtractContentRepository ~ err',
          err
        )
      );
  }
  ...
  deleteVideo(video: YoutubeVideo) {
    return this.firestoreRepository
      .deleteUsersDocument(this.collectionPath, video.id)
      .pipe(
        catchError((err) => {
          console.log(
            '❤️‍🔥 ~ file: extractcontent.repo.ts ~ line 76 ~ ExtractContentRepository ~ catchError ~ err',
            err
          );
          throw new Error(err);
        })
